...
function initBasicAnimations() {
  ...
  const animateElements = document.querySelectorAll(
    ".highlight-card, .path-step, .tool-card, .testimonial-card, .faq-item"
  );
...
function initScrollEffects() {
  ...
  window.addEventListener("scroll", () => {
    ...
    if (heroSection) {
      ...
      if (logoGlow) {
        logoGlow.style.transform = `translate(-50%, -50%) scale(${
          1 + scrolled * 0.0001
        })`;
      }
    }
...
function initCardAnimations() {
  ...
  gsap.utils.toArray(".card-icon").forEach((icon) => {
    // Skip auth-container icons to prevent floating login form
    if (icon.closest(".auth-container")) {
      return;
    }
