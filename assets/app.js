...
function showInstallPrompt() {
    ...
    if (isIOS) {
        let browserMessage = '';
        if (!isSafari) {
            browserMessage = '<p style="color: #ef4444; font-size: 0.9rem; margin-top: 0.5rem;"><i class="bi bi-exclamation-triangle"></i> For best experience, use Safari browser</p>';
        }
        ...
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) { // Element node
                    const newToolsLinks = node.querySelectorAll ? node.querySelectorAll('a[href="/tools.php"]') : [];
