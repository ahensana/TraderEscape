...
class TradingBackground {
    ...
    createSupportResistance(container) {
        ...
        for (let i = 0; i < lineCount; i++) {
            ...
            line.style.cssText = `
                ...
                width: ${length}px;
                height: 1px;
                background: linear-gradient(90deg, transparent, ${isSupport ? '#10b981' : '#ef4444'}, transparent);
                opacity: 0.25;
                transform: rotate(${angle}deg);
    ...
    updateScrollEffect() {
        ...
        const supportResistance = document.querySelectorAll('.trading-support-resistance');
        ...
        fibonacci.forEach((level, index) => {
            ...
            level.style.filter = `drop-shadow(0 0 ${3 + glow * 5}px rgba(245, 158, 11, 0.3))`;
        });
    }
    ...
    updateBackgroundHeight() {
        ...
        if (background) {
            ...
            const bgElements = background.querySelectorAll('.bg-grid, .bg-particles, .bg-trading-elements, .bg-glow');
